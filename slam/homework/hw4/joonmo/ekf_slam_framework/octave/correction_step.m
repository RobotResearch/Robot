...
for i = 1:m
	...
	if(observedLandmarks(landmarkId)==false)
		...
		mu(3+2*landmarkId-1:3+2*landmarkId,1) = mu(1:2,1) + z(i).range*[cos(z(i).bearing + mu(3,1)); sin(z(i).bearing + mu(3,1))];
  ...
  expectedZ(2*i-1:2*i,1) = [sqrt(q); atan2(delta_y, delta_x) - mu(3)];
	% TODO: Compute the Jacobian Hi of the measurement function h for this observation
	Hi_low = 1/q*[-sqrt(q)*delta_x, -sqrt(q)*delta_y, 0, sqrt(q)*delta_x, sqrt(q)*delta_y;
            delta_y, -delta_x, -q, -delta_y, delta_x];
	Fx = zeros(5, size(mu,1));
  Fx(1:3,1:3) = eye(3);
  Fx(4:5, 3+2*landmarkId-1:3+2*landmarkId) = eye(2);
  Hi = Hi_low*Fx;

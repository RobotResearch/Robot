...
for i = 1:m
 ...
 if(observedLandmarks(landmarkId)==false)
  % TODO: Initialize its pose in mu based on the measurement and the current robot pose:
    meas = zeros(2,1);
    meas(1) = z(i).range*cos(z(i).bearing+mu(3));
    meas(2) = z(i).range*sin(z(i).bearing+mu(3));
   mu(3 + 2*j - 1) = mu(1) + meas(1);
    mu(3 + 2*j) = mu(2) + meas(2);
 ...
 % to compute the corresponding expected measurement in expectedZ:
  delta_x = mu(3 + 2*j - 1) - mu(1);
  delta_y = mu(3 + 2*j) - mu(2);
  q = delta_x^2 + delta_y^2;

  expectedZ(2*i-1) = sqrt(q);
  expectedZ(2*i) = atan2(delta_y, delta_x) - mu(3);
